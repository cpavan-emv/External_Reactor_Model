...
Ndatcycle=round(Int,length(t)/Ncycle)
cols=palette(:rainbow, Ncycle);
maxV=maximum(V)
begin
    plt=Plots.plot()
for i in 1: Ncycle
    rng=(i-1)*Ndatcycle+1:i*Ndatcycle
    plot!(plt,V[rng]/maximum(V), yB[rng,1]/100e3, #xaxis=:log, yaxis=:log,
        color=cols[i])
end
plot!(plt, xlims=(0.05,0.2), ylims=(10,25))
plot(plt, legend=:none)
end

import BSON, DataFrames
file=@sprintf("output/data/topen=%udeg_Vc=%.1fVd.bson",Int(CA_open), beta)
BSON.bson(file,Dict(:t=>t, :u=>u))