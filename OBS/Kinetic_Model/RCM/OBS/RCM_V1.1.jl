# This file only has the reactions (V1.0 has a non-reacting version)
...
function f2!(du, u, p, t)
    ...
    du .= M2\vcat([-5e5*(u[2]-(250+273.15)); -fluid_props.rho(gasB)*fluid_props.enthalpy(gasB)*dV; -fluid_props.rho(gasB)*dV],
...
sol = solve(prob,CNLF(autodiff=false),p=param)
#sol = solve(prob,Rodas5P(autodiff=false),p=param, reltol=1e-6)
