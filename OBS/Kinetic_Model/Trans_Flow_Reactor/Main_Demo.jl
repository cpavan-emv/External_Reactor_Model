...
using Plots
plt=plot(x,u[1,:].-273.15, label="IVP SS solution")

u0_discrete=utmp[:]
[DE_Model.set_gas_state!(g,c, param[:Pressure]) for (g,c) in 
    zip(param[:gasses], eachcol(reshape(u0_discrete,(DE_Model.NvNc...))))]
using NLsolve
...
for i in 1:2
    if i==1
        case="Spatially Discretized SS Solution"
    else
        case="Perturbed SS Solution"
    param[:mdot_A]=1.2
...
plot!(plt,x,u2[1,:].-273.15, label=case)
end
...
function P_dP(t)
    if t<20.0
        return [P0 + 5*0.5(1-cos(t*π/20)); 5*0.5*sin(t*π/20)*π/20]*100e3
    else
        return [P0+5; 0]*100e3
    end
end

savet=[collect(1:4);collect(8:4:40)]

param[:P] = (t) -> P_dP(t)[1] # vary pressure from P0 to P0+5 bar
param[:dP_dt]= (t) -> P_dP(t)[2]
fun_trans(du,u,t)=DE_Model.f_trans!(du,u,(param,),t)
trans_prob=DE_Model.ODE.ODEProblem(DE_Model.f_trans!,u0_discrete,(0.0,40.0))
sol_trans=DE_Model.ODE.solve(trans_prob, DE_Model.ODE.Tsit5(),p=(param,),
    reltol=1e-6, abstol=1e-8,saveat=savet)
