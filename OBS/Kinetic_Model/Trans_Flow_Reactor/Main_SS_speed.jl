...
M=DE_Model.Mcom(gas, 1.0)
F=DE_Model.Fcom(gas)
B=DE_Model.Bcom(gas)
D=DE_Model.Dcom(gas)
Mass=DE_Model.Mass_com(gas, 1.0)
...
###################################
u0=reshape(stack(fill([fluid_props.enthalpy(gas);
    fluid_props.Y(gas)[DE_Model.spec_ind[1:end-1]]], Ncell)), (Ncell*Nvar,1))[:]
...
param=Dict(:Pressure => P0*100e3,
    ...
    :gas=>gas,
    :rhocat=>1000,
    :inlet_state=>u0[1:Nvar],
    :Twall=>250+273.15,
    :gasses=>gasses)

du=copy(u0)
#flux=DE_Model.f_SS!(du,u0, (param,))
...
sol=DE_Model.ODE.solve(prob, DE_Model.ODE.RadauIIA5(autodiff=false),
    saveat=x, p=(param,))
utmp=stack(sol.u)
u=Matrix{Float64}(undef, size(utmp,1)+2, size(utmp,2))
spec_ind=DE_Model.spec_ind
for i in axes(utmp,2)
    fluid_props.setHPY(gas, (utmp[1,i], param[:Pressure], utmp[2:end,i]),spec_ind)
    u[1,i]=param[:mdot_A] ./ fluid_props.rho(gas)
    u[2,i] = fluid_props.T(gas)
    u[3:end,i] .= fluid_props.X(gas)[spec_ind]
end
...
#J1=spzeros(Nvar*Ncell, Nvar*Ncell)
#@btime DE_Model.j_SS!(J1,u0,(param,))
#J2=Matrix(0*J1)
#@time DE_Model.jacobian_brute_force(J2,u0,(param,))

u0_discrete=[u[1,:]';utmp]
# update Nvar
if DE_Model.NvNc[1] != size(u0_discrete,1)
    DE_Model.NvNc[1] = size(u0_discrete,1)
...
Jac(J1,u0)
J2=similar(J1)
DE_Model.jacobian_brute_force(J2,u0,(param,),DE_Model.f_SS!)
...
sol2=NLsolve.nlsolve(fun, Jac2,u0, iterations=5,method=:newton,
    ftol=1e-3)


utmp2=reshape(sol2.zero, (DE_Model.NvNc...))
u2=Matrix{Float64}(undef, size(utmp2,1)+1, size(utmp2,2))
for i in axes(utmp,2)
    fluid_props.setHPY(gas, (utmp2[2,i], param[:Pressure], utmp2[3:end,i]),spec_ind)
